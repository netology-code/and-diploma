...
1. **Создать мобильное приложение на свою тему**.
1. **Создать мобильное приложение для водителя транспортной компании от партнёра O2RUS базовую версию**.
1. **Создать мобильное приложение для водителя транспортной компании от партнёра O2RUS расширенную версию**.
...
<details>
  <summary> 3. Для тех, кто разрабатывает работу для водителя транспортной компании от партнёра O2RUS базовая версия</summary>

### Цель проекта:
Цель дипломного проекта - разработать мобильное приложение для водителя транспортной компании. Мобильное приложение должно простраивать маршрут следования по заранее построенному треку ТС. Координаты трека доступны по API ключу. 

В результате выполнения этого задания вы:
- получите опыт разработки мобильного приложения на основе технического задания (ТЗ);
- примените знания вёрстки интерфейсов 
- получите возможность использовать созданный проект в своём портфолио разработчика (с условием размещения в репозитории O2MoveStudent, без публичного доступа к коду)
- сможете попасть на практику к партнеру «О2RUS»  с возможностью дальнейшего трудоустройства
...
### Инструменты / дополнительные материалы, которые могут пригодиться для выполнения задания:
- [Android Studio](https://developer.android.com/studio).
- [Kotlin](https://kotlinlang.org/).
- [Retrofit](https://square.github.io/retrofit/).
- [Dagger Hilt](https://dagger.dev/hilt/).
...
#### Требования к интерфейсу:
Вам необходимо разработать мобильное приложение для водителя транспортной компании. Интерфейс  должен содержать: 

1.	Карта с местоположение ТС и маршрут его следования на главной странице.  
1.	Текущие поставки. Список маршрутов, назначенных логистом с информацией: откуда > куда, номер заявки, статус заявки, расчетное время поставки, равное количеству дней (данные доступны по API ключу). 

    В заявке должны быть кнопки, которые будут менять статус ТС: 
    1.	“Взял в работу” - построить маршрут к месту погрузки. 
    1.	“В дороге к месту погрузки”.
    1.	“Машина загружена” - построен маршрут к месту разгрузки. 
    1.	“В дороге на место разгрузки”.
    1.	“На стоянке” -  ТС на стоянке. 
    1.	“ТС прибыло на место разгрузки” – машина приехала на разгрузку. 
    1.	“Машина разгружена” – после разгрузки ТС. 
    1.	“Забрал документы” - пока водитель не забрал документы и не подтвердил это, он не может получить данные по следующей заявке логиста в меню “Текущие поставки”.             
1. История поставок. История с информацией по выполненным доставкам: откуда > куда, время в дороге, название контрагента, место загрузки, место разгрузки (данные доступны по API ключу).  
1. Связаться с разработчиками. Написать сообщение разработчику, позвонить разработчику. 
1. Мой профиль. Информация о водителе: ФИО, колонный, номер телефона водителя, номер телефона колонного, номер телефона логиста (данные доступны по API ключу).   

Примеры оформления интерфейса можно посмотреть в [приложении к заданию](https://github.com/netology-code/and-diploma/blob/FAN-40/README.md#приложение-к-заданию).

#### Технические требования:
-	Наличие аутентификации, регистрации и возможности разлогиниться
-	Наличие страницы управления учетной записью: изменений email, логина, пароля
-	Поддержка медиа-файлов: возможность загружать полученные документы,  смотреть документы / фото
-	Наличие нетривиальной бизнес-логики
-	API для аутентифицированного пользователя 
-	Работа с внешним API (в зависимости  от бизнес-логики)
-	Наличие документации 
    

#### Общие требования к проекту:
Обязательная часть:
1.	Проект собирается без ошибок. Количество ворнингов(предупреждений)  сведено к минимуму.
2.	Для вёрстки могут использоваться сторонние библиотеки.
3.	Верстка адаптивная, экраны приложения хорошо выглядят на разных устройствах.
4.	Для цветов и шрифтов используется стайлгайд, все настройки собраны в одном месте.
5.	Для всех модулей используется одна архитектура.
6.	В проекте применяется корректная работа с зависимостями для сборки модулей и использования сервисов.
7.	В реализации копирование кода минимально.
8.	Код написан в одном стиле. Обратите внимание на форматирование, именование классов, свойств и функций.
9.	В репозитории проекта оформлено README: описание основного функционала, архитектуры, скриншоты интерфейса, описание проекта по вашему усмотрению. Это поможет при проверке работы и при демонстрации проекта будущему работодателю.

Необязательная часть:
1.	В проект добавлены комментарии к коду: описание основных модулей, дополнения к неочевидным моментам реализации, описание бизнес-логики.
2.	Все строки в интерфейсе приложения  локализованы.
3.	В приложении реализована поддержка тёмной темы.
4.	Сервисы с бизнес-логикой покрыты unit-тестами.

### Критерии оценки проекта:
-	Результаты работы должны быть сданы в виде ссылки на публичный репозиторий с кодом на github.com.
-	В корневой папке репозитория должен  содержаться файл README.md с описанием проекта
...
-	Оценке подлежит также внешний вид приложения и удобство пользования им. Несоблюдение размеров, цветов и пропорций из figma могут быть основанием для отправки на доработку.
-	При проверке приложение должно быть работоспособным в объёме реализованного обязательного функционала. 

### Отправка работы на проверку
Чтобы отправить работу на проверку, загрузите репозиторий на Github и в личном кабинете студента отправьте ссылку на проект.

### Приложение к заданию

Примеры оформления интерфейса:

![](/pic/o2rus_order_details.png)

![](/pic/o2rus_order_create.png)

![](/pic/o2rus_driver_map.png)

![](/pic/o2rus_order_confirmed.png)

![](/pic/o2rus_drawer.png)

![](/pic/o2rus_orders.png)
...
<details>
  <summary> 4. Для тех, кто разрабатывает работу для водителя транспортной компании от партнёра O2RUS расширенная версия</summary>

### Цель проекта:
Цель дипломного проекта – разработать мобильное приложение по осмотру ТС, состоящее из одиннадцати простых страниц, информацию нужно подтягивать и передавать в основной проект по АПИ ключам.  

В результате выполнения этого задания вы:
- получите опыт разработки мобильного приложения
- примените знания вёрстки интерфейсов
- получите возможность использовать созданный проект в своём портфолио разработчика (с условием размещения в репозитории O2MoveStudent, без публичного доступа к коду)
- сможете попасть на оплачиваемую стажировку к партнеру «О2RUS» для продолжения разработки и поддержки приложения c возможностью дальнейшего трудоустройства
...
- [Figma](https://figma.com/).
- [Git](https://git-scm.com/) + [GitHub](https://github.com/).
- Сайт компании https://o2move.ru/
- API основной разработки -  https://test.o2move.ru/ (доступ после подписания  соглашения о неразглашении)
...
Требования к проекту:
Нужно разработать мобильное приложение в простой цветовой гамме, возможно чёрно-белое или в похожей цветовой гамме.
Приложение является дополнением к  основному сайту и будет использоваться в случае отсутствия интернета.
В приложении к заданию  доступны видео с демонстрацией работы данного функционала на сайте заказчика. оформления интерфейса можно посмотреть .

1.	Основное меню должно содержать кнопки:
    - Прием / Передача тягача
    - Прием / Передача ППЦ
    - Осмотр тягача
    - Осмотр ППЦ
    - Запланированные осмотры
    - Заметки
    - Документы
    - Написать разработчику
    - Выйти
      
    Кнопка “”Осмотр тягача” должна быть активной.

  	![](/pic/o2rus_inspect_tractor.png)
1. Добавить таблицу пройденных осмотров с фильтрацией

   ![](/pic/o2rus_inspections_list.png)
   Вверху добавить кнопку “+” для создания нового осмотра и  фильтрации по датам, колонному, базе осмотра, автомобилю, ППЦ, водителю.

   ![](/pic/o2rus_inspections_list_filer.png)
1. Создание осмотра Транспортного Средства ( далее ТС). 

   В БД каждое ТС по id в составе автопоезд + водитель привязаны к колонному. Поля формы осмотра:
    - первое поле - гос.номер,
    - второе - ППЦ,
    - третье  - дата и время,
    - четвертое  - выбор базы осмотра,
    - пятое - колонный,
    - шестое - водитель,
    - седьмое - номер водителя,
    - восьмое - уточняющий вопрос: водитель на осмотре?
    - девятое - начало осмотра.
    
    ![](/pic/o2rus_create_tractor_inspection.png)
1. Жалобы водителя.

   Три поля для ввода текста, 260 символов. На каждой странице в шапке отображается номер ТС, всегда есть кнопки  “далее” и “назад”. Можно вернуться к уже написанной жалобе и внести изменения, которые перезапишутся в БД.

   ![](/pic/o2rus_driver_complaint.png)
1. Комплектность тягача
    - Меню состоит из вопросов с ответами “да” и “нет”, фото платона, показаний по одометру,  фото СТС и кабины.
    - Номера топливных карт, ТО ТС, ОСАГО, мап-карту получаем по API.
    - В комплектности тягача должно быть поле с датой предыдущего осмотра, а с левой стороны предыдущий ответ по комплектности ТС.

      <p float="left">
       <img src="/pic/o2rus_tractor_completeness_1.png" width="400" />
       <img src="/pic/o2rus_tractor_completeness_2.png" width=400" /> 
      </p>
    - При наличии платона должно появляться поле для фото платона

      ![](/pic/o2rus_platon.png)
    - При остутствии платона должно появляться текстовое поле для заполнения

      ![](/pic/o2rus_platon_absent.png)
    - Показания одометра - еще одно обязательное поле для заполнения

      ![](/pic/o2rus_odometer.png)
    - Фото кабины -  обязательное для заполнения поле
      ![](/pic/o2rus_cabin_photo.png)
    - Фото СТС -  обязательное для заполнения поле при наличии СТС, при отсутствии -  должно появляться поле для комментария, аналогично платону

      ![](/pic/o2rus_ctc_photo.png)
1. Общее фото тягача

   ![](/pic/o2rus_tractor_general_photo.png)
1. Осмотр повреждений тягача.

   35 повреждений по списку  + 2 повреждения “Другое 1” и “Другое 2”.

   При выявленном повреждении, можно добавить фото (выбрать файл) + поле для ввода комментария при необходимости.

   ![](/pic/o2rus_tractor_damages.png)
1. Осмотр шин тягача.

   Давление и остаток протектора -  обязательные поля для ввода цифровых значений
   Марка, С/Н шины -  поле разворачивается для выбора серийного номера шины
   Если номер шины нечитаем или не соответствует месту установки колеса, должно появляться поле на выбор: не читабелен, не соответствует
   Сделать фото – обязательное поле с возможностью загрузки файла. Все фото с осмотра ТС отправляются на облако по API.

   ![](/pic/o2rus_tires_inspection.png)
1. Комментарий воротного механика

   ![](/pic/o2rus_mechanic_comment.png)
1. Завершающая часть осмотра ТС

   Ввод гос. номера и ФИО водителя
   Добавить переключатель “подпись водителя”
   Кнопки выбора “Завершить осмотр: да, нет
   При завершении осмотра ТС нужно добавить на последнюю страницу кнопку конвертации в PDF с возможностью сохранить его в папку на мобильном устройстве.

   ![](/pic/o2rus_tractor_inspection_complete.png)
1. По завершении осмотра возвращаемся в таблицу осмотров ТС.
   Завершенный осмотр в приложение подтягивать не обязательно.

### Критерии оценки проекта:
- Результаты работы должны быть сданы в виде ссылки на публичный репозиторий с установочным APK на github.com
- В корневой папке репозитория должен обязательно содержаться файл README.md с описанием проекта
- В сданной работе должен быть полностью реализован соответствующий требованиям функционал из разделов, помеченных как обязательные, и может быть частично или полностью реализован дополнительный функционал.
- Оценке подлежит также внешний вид приложения и удобство пользования им.
- При проверке приложение должно быть работоспособным в объёме реализованного обязательного функционала.

Задание может быть возвращено на доработку при несооответствии любому из вышеуказанных пунктов

### Отправка работы на проверку

Отправьте ссылку на Github в личном кабинете студента.  

### Приложение к заданию

Демонстрация работы функционала по осмотру ТС на сайте:
[Видео 1](https://disk.yandex.ru/i/XpCcx_cMz5TauA)
[Видео 2](https://disk.yandex.ru/i/gpQxBj06_ojjOA)
